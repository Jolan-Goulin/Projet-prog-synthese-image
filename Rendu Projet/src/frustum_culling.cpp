...
bool LOD(Node* node, Camera c, int lod){
    ...
    p.z = p.z/15;
    Point3D o = createPoint(c.origin.x, c.origin.y, -c.origin.z/15);
    Vector3D v =  createVectorFromPoints(p, o);
    int distance = norm(v);
    if (distance < lod-node->data.deep*10){
        return false;
    }
    else{
        return true;
