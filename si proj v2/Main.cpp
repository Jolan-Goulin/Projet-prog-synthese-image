...
bool lines = false;
QT.initMap (Map, Xsize, int Ysize);
...
void quadtreeTravelDraw(Camera c, Node* Qt, Point3D sun){
    ...
    if(Qt->isLeaf()){
        if(lines){
            ...
            drawTriangle(t1, sun, createColor(25/255, 200/255, 25/255));
            drawTriangle(t2, sun, createColor(25/255, 200/255, 25/255));
        }
    }
    else if(LOD(Qt, c)){
        if(lines){
...
int main(int argc, char** argv) 
{
    ...
    while(loop) 
    {
        ...
        geometryCamera(&cam ,  zFar,  zNear,  cam.fov);
        glPushMatrix();
            glTranslatef(-cam.origin.x, -cam.origin.y, -cam.origin.z);
            glRotatef(cam.phi,0,0,1);
            glRotatef(cam.ro,sin(cam.phi),-cos(cam.phi),0);
